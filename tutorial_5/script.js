...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    displayButton.addEventListener('click', () => {
        ...
        restaurants.forEach((resto, index) => {
            restaurantList.innerHTML += `<div class="restaurant-item">
                <span class="restaurant-name">${resto.name}</span>:
                <span class="restaurant-cuisine">${resto.cuisine}</span>
            </div>`;
        });
    ...
    filterButton.addEventListener('click', (event) =>{
        ...
        cheapRestaurants.forEach((resto, index) => {
            filteredList.innerHTML += `<div class="restaurant-item">
                <span class="restaurant-name">${resto.name}</span>:
                <span class="restaurant-cuisine">${resto.cuisine}</span>
            </div>`;
        });
    ...
    mapButton.addEventListener('click', (event) => {
        ...
        const namesListUL = document.createElement('ul');

        names.forEach((name) => {
            const listItem = document.createElement('li');
            listItem.textContent = name;
            listItem.classList.add('restaurant-name');
            namesListUL.appendChild(listItem);
        });

        mappedList.appendChild(namesListUL);
    ...
    findButton.addEventListener('click', (event) =>{
        ...
        // Step 1: Use find to get the restaurant with the highest rating
        ...
        if (bestRestaurant) {
            foundItem.innerHTML = `
                <h2 class="restaurant-name">Name: ${bestRestaurant.name}</h2>
                <div class="restaurant-cuisine">Cuisine: ${bestRestaurant.cuisine}</div>
                <div class="restaurant-rating">Rating: ${bestRestaurant.rating}</div>

                <div class="restaurant-price">Price: ${bestRestaurant.priceRange}</div>
                <div class="restaurant-cuisine">Location: ${bestRestaurant.neighborhood}</div>
            `;
        } else {
