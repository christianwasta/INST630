...
const getName = () => {
  ...
  return raw || 'Someone';
  // What does || do here? If raw is an empty string (falsy), return 'Someone'. if TRUTHY then return right first // 
...
const getGuests = () => Number.parseInt(guestInput.value, 10) || 0;   //parsing ints and removing negative/decimals
...
btnYes.addEventListener('click', () => {
  ...
  //do this with toggle instead
  btnYes.classList.toggle('active');
...
btnNo.addEventListener('click', () => {
  ...
  regret.textContent = `${getName()} is not going.`;
...
guestInput.addEventListener('input', () => {
  ...
  guestInput.value = guestInput.value.replace(/\D/g, ''); //lookedup
  if (isGoing) {
    updateConfirmation();
  }
