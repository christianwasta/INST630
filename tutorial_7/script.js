...
function createMyChart() {
    ...
    restaurants.forEach(function(restaurant) {
        const cuisine = restaurant.cuisine;
        if (cuisine) {
            cuisineName[cuisine] = restaurant.name;
        }
    });
        ...
        myChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    //https://www.chartjs.org/docs/latest/configuration/tooltip.html#:~:text=in%20the%20tooltip.-,label,Returns%20text%20to%20render%20for%20an%20individual%20item%20in%20the%20tooltip.,-more...
                    //so this only works if 1 restaurant per cuisine. Welp.
                    tooltip: {
                        callbacks: {
                            label: function(tooltipItem) {
                                const cuisine = tooltipItem.label;
                                const restaurant = cuisineName[cuisine];
...
function createMyMap() {
    ...
    try {
        ...
        restaurants.forEach(function(restaurant, index) {
            ...
            // const popupContent = ...;
            const popupContent = `Name: ${restaurant.name}, Cuisine:${restaurant.cuisine}, Rating: ${restaurant.rating}`
